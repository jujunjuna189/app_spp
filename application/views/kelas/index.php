            ...
            <div class="table-responsive mt-4 rounded-4 border border-dark-subtle">
                <table class="table border-dark-subtle">
                    ...
                    <tbody class="fw-semibold text-dark-emphasis">
                        <?php foreach ($kelas as $index => $val) : ?>
                            <tr>
                                <td><?= $index += 1 ?></td>
                ...
                <form action="<?= base_url('kelas/store') ?>" method="post">
                    ...
                    <div class="px-3">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="nama_kelas" id="nama_kelas" class="form-control fw-semibold" placeholder="...">
                        </div>
                    </div>
...
<script>
    const config = {
        update_url: '<?= base_url('kelas/get-by-id?id=') ?>',
        delete_url: '<?= base_url('kelas/delete?id=') ?>',
    };
    const modal = {
        add: '#modal-add',
        update: '#modal-edit',
    ...
    const setFieldForm = (data) => {
        ...
        $(modal.update + ' ' + field.nama_kelas).val(data.nama_kelas);
    }
